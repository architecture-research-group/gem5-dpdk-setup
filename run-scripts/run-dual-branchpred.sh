...
TEMP=$(getopt -o 'h' --long take-checkpoint,num-nics:,script:,branch-pred:,drivenode-script:,freq:,help -n 'dpdk-loadgen' -- "$@")
...
if [[ -n "$checkpoint" ]]; then
  ...
  RUNDIR=${GIT_ROOT}/rundir/iperf-branchpred-exp-3/$num_nics"NIC"-$DRIVE_SCRIPT-$Freq"-ddio-enabled"-$BRANCH_PRED
  setup_dirs
# /dpdk-testpmd-freq-scaling-test
  ...
  CONFIGARGS="$CACHE_CONFIG $CPU_CONFIG -r 3 --cpu-clock=$Freq --bp-type=$BRANCH_PRED" # \
