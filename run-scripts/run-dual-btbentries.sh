...
TEMP=$(getopt -o 'h' --long take-checkpoint,num-nics:,script:,btb-entries:,drivenode-script:,freq:,help -n 'dpdk-loadgen' -- "$@")
...
if [[ -n "$checkpoint" ]]; then
  ...
  RUNDIR=${GIT_ROOT}/rundir/iperf-btbentries-exp-3/$num_nics"NIC"-$DRIVE_SCRIPT-$Freq"-ddio-enabled"-$BTB_ENTRIES
  setup_dirs
# /dpdk-testpmd-freq-scaling-test
  ...
  CONFIGARGS="$CACHE_CONFIG $CPU_CONFIG -r 3 --cpu-clock=$Freq --param=testsys.switch_cpus[0:4].branchPred.BTBEntries=$BTB_ENTRIES" # \
