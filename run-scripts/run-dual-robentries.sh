...
CPU_CONFIG="--param=testsys.cpu[0:4].l2cache.mshrs=46 --param=testsys.cpu[0:4].dcache.mshrs=20 \
  ...
  --param=testsys.switch_cpus[0:4].branchPred.BTBEntries=8192 --param=testsys.switch_cpus[0:4].numIQEntries=120 \
  ...
  --param=testsys.switch_cpus[0:4].issueWidth=8 \
...
function run_simulation {
  ...
  --num-cpus=$(($num_nics)) --mem-type=DDR4_2400_16x4 --mem-channels=4 --mem-size=512MB --script="$GUEST_SCRIPT_DIR/$GUEST_SCRIPT" \
...
TEMP=$(getopt -o 'h' --long take-checkpoint,num-nics:,script:,rob-entries:,drivenode-script:,freq:,help -n 'dpdk-loadgen' -- "$@")
...
CKPT_DIR=${GIT_ROOT}/ckpts-with-baseconfig-1024-rx-tx-ring-3s/$num_nics"NIC"-$DRIVE_SCRIPT
...
if [[ -n "$checkpoint" ]]; then
  ...
  RUNDIR=${GIT_ROOT}/rundir/ISPASS-2024/$num_nics"NIC-ckp"-$DRIVE_SCRIPT
  setup_dirs
  ...
  CONFIGARGS="--max-checkpoints 3 --cpu-clock=$Freq"
  ...
  RUNDIR=${GIT_ROOT}/rundir/iperf-robentries-exp-3/$num_nics"NIC"-$DRIVE_SCRIPT-$Freq"-ddio-enabled"-$ROB_ENTRIES
  setup_dirs
# /dpdk-testpmd-freq-scaling-test
  ...
  CONFIGARGS="$CACHE_CONFIG $CPU_CONFIG -r 3 --cpu-clock=$Freq --param=testsys.switch_cpus[0:4].numROBEntries=$ROB_ENTRIES" # \
