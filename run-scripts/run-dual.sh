...
function run_simulation {
  ...
  --num-cpus=$(($num_nics)) --script="$GUEST_SCRIPT_DIR/$GUEST_SCRIPT" \
...
if [[ -n "$checkpoint" ]]; then
  RUNDIR=${GIT_ROOT}/rundir/$num_nics"NIC-ckp"-$DRIVE_SCRIPT
  # RUNDIR=${GIT_ROOT}/rundir/ISPASS-2024-ckpts-with-baseconfig-1024-rx-tx-ring-test-3s/$num_nics"NIC-ckp"-$DRIVE_SCRIPT
  setup_dirs
  echo "Taking Checkpoint for NICs=$num_nics" >&2
  GEM5TYPE="fast"
  CPUTYPE="AtomicSimpleCPU"
  CONFIGARGS="--max-checkpoints 3 --cpu-clock=$Freq"
  # CONFIGARGS="--max-checkpoints 4 $DEBUB_FLAG --cpu-clock=$Freq"
