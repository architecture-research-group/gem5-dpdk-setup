...
if [[ -n "$checkpoint" ]]; then
  ...
  RUNDIR=${GIT_ROOT}/rundir/memcachedKernel-dualmode/$num_nics"NIC-ckp"-$DRIVE_SCRIPT
  setup_dirs
  ...
  CONFIGARGS="--max-checkpoints 4 $CACHE_CONFIG --cpu-clock=$Freq"
  # CONFIGARGS="-r 1 --max-checkpoints 3 --cpu-clock=$Freq"
  run_simulation > ${RUNDIR}/simout
  exit 0
else
  ((RATE = PACKET_RATE * PACKET_SIZE * 8 / 1024 / 1024 / 1024))
  RUNDIR=${GIT_ROOT}/rundir/memcachedKernel-dualmode/$num_nics"NIC"-$GUEST_SCRIPT-$Freq"-ddio-enabled"
  setup_dirs
