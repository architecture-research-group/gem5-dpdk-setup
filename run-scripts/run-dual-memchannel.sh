...
function run_simulation {
  ...
  --num-cpus=$(($num_nics)) --mem-type=DDR4_2400_16x4 --mem-size=512MB --script="$GUEST_SCRIPT_DIR/$GUEST_SCRIPT" \
...
TEMP=$(getopt -o 'h' --long take-checkpoint,num-nics:,script:,mem-channel:,drivenode-script:,freq:,help -n 'dpdk-loadgen' -- "$@")
...
CKPT_DIR=${GIT_ROOT}/ckpts-with-baseconfig-1024-rx-tx-ring-2s/$num_nics"NIC"-$DRIVE_SCRIPT
...
if [[ -n "$checkpoint" ]]; then
  ...
  RUNDIR=${GIT_ROOT}/rundir/iperf-memchannel-exp-ddio-disabled/$num_nics"NIC"-$DRIVE_SCRIPT-$Freq"-ddio-disabled"-$MEM_CHANNEL
  setup_dirs
# /dpdk-testpmd-freq-scaling-test
  ...
  CONFIGARGS="$CACHE_CONFIG $CPU_CONFIG -r 3 --cpu-clock=$Freq  --mem-channels=$MEM_CHANNEL" # \
