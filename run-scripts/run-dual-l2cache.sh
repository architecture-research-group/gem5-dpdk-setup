...
TEMP=$(getopt -o 'h' --long take-checkpoint,num-nics:,script:,l2-size:,drivenode-script:,freq:,help -n 'dpdk-loadgen' -- "$@")
...
if [[ -n "$checkpoint" ]]; then
  ...
  RUNDIR=${GIT_ROOT}/rundir/iperf-l2cache-exp-3/$num_nics"NIC"-$DRIVE_SCRIPT-$Freq"-ddio-enabled"-$L2_SIZE
  setup_dirs
# /dpdk-testpmd-freq-scaling-test
  ...
  CONFIGARGS="$CACHE_CONFIG $CPU_CONFIG -r 3 --cpu-clock=$Freq --l2_size=$L2_SIZE" # \
