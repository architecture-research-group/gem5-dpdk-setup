...
TEMP=$(getopt -o 'h' --long take-checkpoint,num-nics:,script:,packet-rate:,packet-size:,btb-entries:,loadgen-find-bw,freq:,help -n 'dpdk-loadgen' -- "$@")
...
if [[ -n "$checkpoint" ]]; then
  ...
  --param=system.switch_cpus[0:4].branchPred.BTBEntries=$BTB_ENTRIES --param=system.switch_cpus[0:4].issueWidth=8 \
  ...
  RUNDIR=${GIT_ROOT}/rundir/dpdk-testpmd-btbentries-droprate/$num_nics"NIC-"$PACKET_SIZE"SIZE-"$PACKET_RATE"RATE-"$RATE"Gbps-ddio-enabled"-$GUEST_SCRIPT"-btbentries-"$BTB_ENTRIES
  setup_dirs
