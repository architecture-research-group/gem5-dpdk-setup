...
TEMP=$(getopt -o 'h' --long take-checkpoint,num-nics:,script:,l1i-size:,l1d-size:,drivenode-script:,freq:,help -n 'dpdk-loadgen' -- "$@")
...
if [[ -n "$checkpoint" ]]; then
  ...
  RUNDIR=${GIT_ROOT}/rundir/iperf-l1cache-exp-3/$num_nics"NIC"-$DRIVE_SCRIPT-$Freq"-ddio-enabled"-$L1I_SIZE
  setup_dirs
# /dpdk-testpmd-freq-scaling-test
  ...
  CONFIGARGS="$CACHE_CONFIG $CPU_CONFIG -r 3 --cpu-clock=$Freq  --l1i_size=$L1I_SIZE --l1d_size=$L1D_SIZE" #\
