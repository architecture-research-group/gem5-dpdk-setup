...
function run_simulation {
  ...
  --num-cpus=$(($num_nics)) --mem-type=DDR4_2400_16x4 --mem-channels=1 --mem-size=512MB --script="$GUEST_SCRIPT_DIR/$GUEST_SCRIPT" \
...
TEMP=$(getopt -o 'h' --long pcap-filename:,freq:,take-checkpoint,num-nics:,script:,packet-rate:,loadgen-find-bw,help -n 'dpdk-loadgen' -- "$@")
...
CKPT_DIR=${GIT_ROOT}/ckpts/ckpts-with-new-vmlinux/Loadgen/$num_nics"NIC"-$GUEST_SCRIPT
if [[ -z "$num_nics" ]]; then
...
if [[ -n "$checkpoint" ]]; then
  ...
  RUNDIR=${GIT_ROOT}/rundir/ISPASS-2024/Loadgen/$num_nics"NIC-ckp"-$GUEST_SCRIPT
  setup_dirs
  ...
  PCAP_FILENAME="../resources/iperf3_128pkt.pcap"
  ...
  CONFIGARGS="--max-checkpoints 2 --cpu-clock=$FREQ --loadgen-start=6025995323418580 --loadgen-type=Pcap --loadgen_pcap_filename=$PCAP_FILENAME --packet-rate=$PACKET_RATE --loadgen-replymode=$LOADGENREPLAYMODE --loadgen-port-filter=$PORT"
  run_simulation
  exit 0
else
  ...
  PORT=5002    # for iperf3
  # PCAP_FILENAME="../resources-dpdk/"
  ((INCR_INTERVAL = PACKET_RATE / 10))
  RUNDIR=${GIT_ROOT}/rundir/iperf-sim-time-loadgen-exp/$PCAP_FILENAME"-"$FREQ"-ddio-enabled"
  setup_dirs
  PCAP_FILENAME=../resources/$PCAP_FILENAME
  ...
  CONFIGARGS="--cpu-clock=$FREQ $CPU_CONFIG $CACHE_CONFIG -r 2 --loadgen-type=Pcap --loadgen_pcap_filename=$PCAP_FILENAME --loadgen-start=762152131630 --rel-max-tick 5000000000000 --packet-rate=$PACKET_RATE --loadgen-replymode=$LOADGENREPLAYMODE --loadgen-port-filter=$PORT --loadgen-increment-interva=$INCR_INTERVAL"
